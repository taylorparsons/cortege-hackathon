...
## Summary
Generate a native PowerPoint version of the existing reveal.js presentation so the hackathon deck can be opened and edited as `deck.pptx` while preserving the current slide copy and visual system.
...
1. Given the repo contains `deck.html`, When I run the deck export workflow, Then it writes `deck.pptx` in the repo root with 8 slides. (Verifies: FR-001, FR-005)
2. Given the reveal.js deck defines the presentation copy, When the PowerPoint is generated, Then the slide text in `deck.pptx` matches the current slide copy from `deck.html`. (Verifies: FR-002)
...
- FR-001: The repo shall include a Python generator that uses `python-pptx` to create `deck.pptx` from the reveal.js source content in `deck.html`. (Sources: CR-20260321-0801; D-20260321-0801)
...
- FR-005: The workflow shall save the generated file as `deck.pptx` in the repo root. (Sources: CR-20260321-0801)
