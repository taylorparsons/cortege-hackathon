---
name: athena
description: Use for coding tasks that should follow the ATHENA delivery loop: capture the user request, update PRD and traceability docs, then implement and verify one small change at a time.
---

# ATHENA Loop

Use this for feature work, bug fixes, and other code changes that should be tracked through the full ATHENA audit trail in Claude Code.
...
- Run the bundled script: `python3 ~/.claude/skills/athena/scripts/bootstrap_git_audit.py --out docs/audit/git-history.md`
...
For the current request, establish a `FEATURE_ID`.

Default behavior for Claude Code:
- Treat each new customer request as a new feature by default.
- Create a new dated `FEATURE_ID` using `YYYYMMDD-short-name` unless the user explicitly says the request is continuing prior work or the request is clearly a follow-up on the same active feature.
- Do not default unrelated requests into a long-lived feature just because one already exists.
- If you reuse an existing feature, record that decision in `docs/decisions.md`.

Recommended formats:
- `YYYYMMDD-short-name` (default for new requests in Claude Code)
- `NNN-short-name` (only if the repo already uses a numbered feature system and the request clearly belongs in it)
...
- Commit using the helper script:
  - `python3 ~/.claude/skills/athena/scripts/commit_with_traceability.py --feature <FEATURE_ID> --task <T-001> --summary "<what changed>" --cr <CR-...> --decisions <D-...,...> [--paths ... | --docs-only | --all-changes]`
...
Preferred:
- Run: `python3 ~/.claude/skills/athena/scripts/validate_progress_log.py --repo .`
- Run: `python3 ~/.claude/skills/athena/scripts/print_resume_prompt.py --repo .`
